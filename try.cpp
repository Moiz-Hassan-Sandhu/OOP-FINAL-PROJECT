...
void printMenu(const string& title, const string options[], int numOptions) {
    ...
    for (int i = 0; i < numOptions; i++) {
        cout << margin << "║  " << left 
             << setw(boxWidth - 2) << options[i]
             << "║\n";
    }
    cout << margin << "╚" << string(boxWidth, '=') << "╝" << RESET << endl;
}

// Base 1–4 menu options:
const string commonTitle   = " MAIN ACTIONS ";
...
const int commonCount = 4;

// Renders the common block and returns 1–4, or 0 if invalid.
int showCommonMenu(PaidWorkers* pw) {
    printMenu(commonTitle, commonOptions, commonCount);
    cout << "          Enter choice (1–4), or 0 to back: ";
    int c; 
    if (!(cin >> c) || c < 0 || c > 4) {
        cin.clear(); 
        cin.ignore(1, '\n');
...
void DirectorMenu(PaidWorkers* pw) {
    PolicyEngine pe(pw);
    while (true) {
        int c = showCommonMenu(pw);
        switch (c) {
          case 1: show_Message_menu(pw);             break;
          case 2: viewMyTasks(pw);                   break;
          case 3: assignTask(pw);                    break;
          case 4: delegateIncompetentTasks(pw);      break;
          case 0: return;                            // back to main
        }

        // Director-only options 5 & 6
        const string dirTitle    = " DIRECTOR EXTRAS ";
        const string dirOptions[] = {
            "5) Generate Audit Report",
            "6) View All Logs",
            "0) Back to Director Menu"
        };
        printMenu(dirTitle, dirOptions, 3);
        cout << "          Enter choice (5,6 or 0): ";
        int x; cin >> x;
        if      (x == 5) pe.generateAudit();
        else if (x == 6) pe.readAllLogs();
...
void ManagerMenu(PaidWorkers* pw) {
    ...
    while (true) {
        ...
        const string mngTitle     = " MANAGER EXTRAS ";
        const string mngOptions[] = {
            "5) View Team Performance",
            "0) Back to Manager Menu"
        };
        printMenu(mngTitle, mngOptions, 2);
        cout << "          Enter choice (5 or 0): ";
        int x; cin >> x;
        if      (x == 5) pe.viewTeamPerformance();
        else if (x == 0) continue;
        else cout << "Invalid option\n";
    }
}
...
void EmployeeMenu(PaidWorkers* pw) {
    ...
    while (true) {
        ...
        const string empTitle     = " EMPLOYEE EXTRAS ";
        const string empOptions[] = {
            "5) Submit Timesheet",
            "0) Back to Employee Menu"
        };
        printMenu(empTitle, empOptions, 2);
        cout << "          Enter choice (5 or 0): ";
        int x; cin >> x;
        if      (x == 5) pe.submitTimesheet();
        else if (x == 0) continue;
        else cout << "Invalid option\n";
    }
}
...
void JuniorMenu(PaidWorkers* pw) {
    ...
    while (true) {
        ...
        const string jrTitle     = " JUNIOR EXTRAS ";
        const string jrOptions[] = {
            "5) Request Assistance",
            "0) Back to Junior Menu"
        };
        printMenu(jrTitle, jrOptions, 2);
        cout << "          Enter choice (5 or 0): ";
        int x; cin >> x;
        if      (x == 5) pe.requestAssistance();
        else if (x == 0) continue;
        else cout << "Invalid option\n";
    }
}